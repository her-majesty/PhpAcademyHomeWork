...
const fullNames = itDepartment.map(worker => worker.first + ' ' + worker.last);

const filtered = itDepartment.filter(worker => worker.possition === 'Front-end' || worker.possition === 'Back-end').map(worker  => `name: ${worker.first}; position: ${worker.possition}`);
  
const sorted = itDepartment.sort((a, b) => a.born - b.born);

const counter = itDepartment.reduce(function(acc, worker) {

  const position = worker.possition;
